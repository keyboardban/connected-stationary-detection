...
Track ID (`ID switching`) เสื้อยูนิฟอร์มที่มีสีเหมือนกัน และพื้นที่ภายในห้อง
...
6. ใช้ Polygon Exclusion Zone ตัดบุคคลภายในห้องออกจากการคำนวณ
...
บางพื้นที่ในภาพเป็นห้องที่ไม่ควรนำมาคำนวณเวลานิ่ง จึงเพิ่ม polygon:

```python
EXCLUSION_POLYGON = np.array(
    [[280, 150], [430, 150], [720, 1040], [450, 1040]],
    dtype=np.int32,
)
```
...
- Bounding Box อาจคร่อมขอบห้อง แม้ว่าคนจะยืนอยู่ด้านใดด้านหนึ่งชัดเจน
...
| --- | --- |
| เส้น Polygon สีเทา | ขอบเขตห้องที่ไม่ต้องนำมาคำนวณ |
...
- Exclusion Zone ครอบคลุมห้องถูกต้องหรือไม่
